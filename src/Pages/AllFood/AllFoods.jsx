...
const AllFoods = () => {
    ...
    useEffect(() => {
        const getData = async () => {
            const { data } = await axiosSecure(`/all-items?page=${currentPage}&size=${itemPerPage}&filter=${filter}&sort=${sort}&search=${search}`)
            setCard(data)
        }
        getData()
    ...
    useEffect(() => {
        const getData = async () => {
            const { data } = await axiosSecure(`/items-count?filter=${filter}&search=${search}`)
            setCount(data.count)
        }
        getData()
    }, [filter, search])
                    ...
                    <div className='flex flex-col md:flex-row justify-center items-center gap-5'>
                        ...
                        <form onSubmit={handleSearch}>
                            <div className='flex p-1 overflow-hidden border border-blue-300 rounded-lg    focus-within:ring focus-within:ring-opacity-40 focus-within:border-blue-400 focus-within:ring-blue-300'>
                                ...
                                <button
                                    className='btn px-1 md:px-4 py-3 text-sm font-medium tracking-wider text-gray-100  transition-colors duration-300 transform bg-blue-400 rounded-md hover:bg-blue-500 focus:bg-blue-500 focus:outline-none'>
